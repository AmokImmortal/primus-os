...
class PrimusRuntime:
    ...
    def __init__(self):
        ...
        if get_captains_log_manager is not None:
            try:
                self.captains_log_manager = get_captains_log_manager(
                    system_root=self.system_root
                )
            except Exception as exc:  # noqa: BLE001
                logger.warning("Failed to initialize Captain's Log manager: %s", exc)
                self.captains_log_manager = None
    ...
    def _ensure_core(self) -> PrimusCore:
        ...
        if self._core is None:
            ...
            self._core = PrimusCore(
                system_root=self.system_root,
                captains_log_manager=self.captains_log_manager,
            )
    ...
    def chat_once(
        ...
        """Runtime-level wrapper for a single chat turn with optional session and RAG."""
        ...
        if hasattr(core, "chat"):
            ...
            if use_rag is not None:
                kwargs["use_rag"] = use_rag
            if rag_index is not None:
                kwargs["rag_index"] = rag_index
            if max_tokens is not None:
                kwargs["max_tokens"] = max_tokens
            return core.chat(user_message, **kwargs)

        # Fallback to legacy model manager usage if chat() is unavailable.
        if hasattr(core, "model_manager") and hasattr(core.model_manager, "generate"):
            return core.model_manager.generate(user_message, max_tokens=max_tokens or 256)

        raise RuntimeError("No chat-capable core available for chat_once.")
