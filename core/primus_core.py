    ...
    def captains_log(self, action: str, text: str = "") -> str:
        ...
        if action_normalized == "status":
            try:
                ...
                logger.warning("Captain's Log status retrieval failed: %s", exc)
                return "Captain's Log status is currently unavailable."
        ...
        if action_normalized == "write":
            ...
            try:
                ...
                ts = "unknown time"
                if isinstance(entry, dict):
                    ts = entry.get("timestamp") or entry.get("ts") or ts
                return f"Captain's Log entry recorded at {ts}."
            except PermissionError as exc:
                return str(exc)
            except Exception as exc:  # noqa: BLE001
                logger.warning("Captain's Log write failed: %s", exc)
                return f"Captain's Log operation failed for action {action_normalized!r}."
