...
class PrimusCore:
    ...
    def captains_log(self, action: str, text: str = "") -> str:
        """
        High-level Captain's Log API used by primus_cli.py.

        action: 'write', 'read', or 'clear'.
        text  : message to write when action == 'write'.
        """

        if not getattr(self, "captains_log_manager", None):
            return "Captain's Log is not available on this PrimusCore instance."

        action_normalized = action.lower().strip()

        if action_normalized == "write":
            if not text.strip():
                return "No message provided to write."
            try:
                self.captains_log_manager.write_entry(text)  # type: ignore[operator]
                return "OK: entry written to Captain's Log."
            except Exception as exc:  # noqa: BLE001
                logger.warning("Captain's Log write failed: %s", exc)
                return f"Failed to write entry: {exc}"

        if action_normalized == "read":
            try:
                entries = self.captains_log_manager.read_recent(limit=20)  # type: ignore[operator]
            except Exception as exc:  # noqa: BLE001
                logger.warning("Captain's Log read failed: %s", exc)
                return f"Failed to read Captain's Log: {exc}"

            if not entries:
                return "Captain's Log is empty."

            lines = ["Captain's Log (most recent last):"]
            for idx, entry in enumerate(entries, start=1):
                ts = entry.get("ts", "?")
                level = entry.get("level", "INFO")
                msg = entry.get("message", "")
                lines.append(f"{idx:02d}) [{ts}] {level}: {msg}")
            return "\n".join(lines)

        if action_normalized == "clear":
            try:
                self.captains_log_manager.clear()  # type: ignore[operator]
                return "OK: Captain's Log cleared."
            except Exception as exc:  # noqa: BLE001
                logger.warning("Captain's Log clear failed: %s", exc)
                return f"Failed to clear Captain's Log: {exc}"
