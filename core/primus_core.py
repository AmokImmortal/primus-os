...
class PrimusCore:
    ...
    def rag_index_path(self, name: str, path: str, recursive: bool = False) -> None:
        """
        Public wrapper used by the CLI to index a path into a named RAG index.
        Delegates to the underlying RAG indexer/manager.
        """
        ...
        if hasattr(self, "rag_manager") and getattr(self, "rag_manager") is not None:
            self.rag_manager.index_path(name=name, path=path, recursive=recursive)
            return
    ...
    def rag_retrieve(
        ...
        """
        Public wrapper used by the CLI to retrieve top-k documents from a named RAG index.
        Delegates to the underlying retriever; returns (score, doc) pairs.
        """
        ...
        if hasattr(self, "rag_manager") and getattr(self, "rag_manager") is not None:
            logger.warning(
                "rag_retrieve: rag_retriever missing; rag_manager is present but "
