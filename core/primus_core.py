...
class PrimusCore:
    ...
    def _load_history(self, session_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if hasattr(sm, "ensure_session"):
                sm.ensure_session(session_id, owner="user", privacy="private")

            if hasattr(sm, "load_history"):
                return list(sm.load_history(session_id))

            if hasattr(sm, "get_messages"):
                return list(sm.get_messages(session_id))

            session_ref = sm.load_session(session_id)
    ...
    def _append_message(self, session_id: str, role: str, who: str, text: str) -> None:
        ...
        try:
            ...
            if hasattr(sm, "add_message"):
                sm.add_message(session_id, role=role, who=who, text=text)
                return

            # Fallback: load, append, and save when only load/save APIs exist
            if hasattr(sm, "load_history") and hasattr(sm, "save_history"):
                history = list(sm.load_history(session_id))
                history.append({"role": role, "who": who, "text": text})
                sm.save_history(session_id, history)
                return

            if hasattr(sm, "load_session") and hasattr(sm, "_save_to_disk"):
                session_ref = sm.ensure_session(session_id, owner="user", privacy="private") if hasattr(sm, "ensure_session") else sm.load_session(session_id)
                if isinstance(session_ref, dict):
                    session_ref.setdefault("messages", []).append({"role": role, "who": who, "text": text, "ts": datetime.utcnow().isoformat() + "Z"})
                    sm._save_to_disk(session_ref)
        except Exception:
