...
class SessionManager:
    ...
    def _read_session_file(self, session_id: str) -> List[Dict[str, Any]]:
        ...
        # Preferred path: JSON array payload
        try:
            with path.open("r", encoding="utf-8") as handle:
                raw = json.load(handle)
            return self._normalize_messages(raw, session_id)
        except Exception:
            # Fallback: tolerate JSONL-by-line legacy files
            messages: List[Dict[str, Any]] = []
            try:
                with path.open("r", encoding="utf-8") as handle:
                    for line in handle:
    ...
    def _save_history(self, session_id: str, history: List[Dict[str, Any]]) -> None:
        """Persist a full history list to disk (trimmed if needed)."""
    ...
    def append_message(self, session_id: str, msg: Dict[str, Any]) -> None:
        ...
        if not self.session_exists(session_id):
            try:
                ...
                logger.warning("SessionManager.append_message: failed to initialize %r: %s", session_id, exc)
                return
