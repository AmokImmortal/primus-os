        ...
        if isinstance(result, list):
            if not result:
                ...
                for i, hit in enumerate(result, 1):
                    score = hit.get("score")
                    path = hit.get("path") or hit.get("metadata", {}).get("source") or hit.get("metadata", {}).get("path")
                    preview = hit.get("preview") or hit.get("metadata", {}).get("text", "")
                    snippet = preview.strip().splitlines()[0] if preview else ""
                    display_score = f"{score:.4f}" if isinstance(score, (int, float)) else str(score)
                    print(f"  {i}. score={display_score} path={path or '[unknown]'}")
                    if snippet:
                        print(f"     {snippet}")
        else:
            pretty_print_object(result)
...
def cmd_chat(args):
    ...
    if getattr(args, "message", None):
        try:
            ...
            rag_index = args.index or "docs" if use_rag else None
            ...
            elif hasattr(runtime, "chat_once"):
...
def cmd_chat_interactive(args):
    ...
    while True:
        ...
        try:
            ...
            elif hasattr(runtime, "chat_with_options"):
...
def cli_session_list(args: argparse.Namespace) -> None:
    """
    List known session IDs from PrimusCore / SessionManager.
    """
    try:
        from core.primus_runtime import PrimusRuntime
    except Exception as exc:  # noqa: BLE001
        print(f"Runtime not available: {exc}")
        return

    try:
        runtime = PrimusRuntime()
    except Exception as exc:  # noqa: BLE001
        print(f"Failed to initialize PrimusRuntime: {exc}")
        return

    core = runtime._ensure_core() if hasattr(runtime, "_ensure_core") else None
    if not core:
        print("Runtime core is not available.")
        return

    session_mgr = getattr(core, "session_manager", None)
    if session_mgr is None:
        print("Session manager is not available.")
        return

    list_fn = getattr(session_mgr, "list_sessions", None)
    if not callable(list_fn):
        print("Session manager does not support listing sessions.")
        return
...
def cli_captains_log(args: argparse.Namespace) -> None:
    """Captain's Log controls for writing, reading, and clearing entries."""
    ...
    core = runtime._ensure_core() if hasattr(runtime, "_ensure_core") else None
    handler = getattr(core, "captains_log", None) if core else None

    if handler is None:
        print("Captain's Log is not available.")
        return

    action = getattr(args, "cl_command", "").strip().lower()

    try:
        if action == "write":
            result = handler(action="write", text=getattr(args, "text", ""))
        elif action == "read":
            # PrimusCore.captains_log currently accepts action/text only; limit is parsed for future use.
            result = handler(action="read", text="")
        elif action == "clear":
            result = handler(action="clear", text="")
        else:
            print("Unsupported Captain's Log command.")
            return
    except Exception:
        logger.exception("Captain's Log command failed")
        show_trace()
        return
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_cl = subparsers.add_parser("cl", help="Captain's Log controls (write/read/clear)")
    cl_sub = p_cl.add_subparsers(dest="cl_command", required=True)

    cl_write = cl_sub.add_parser("write", help="Append a new entry to Captain's Log")
    cl_write.add_argument("text", help="Text of the log entry")
    cl_write.set_defaults(func=cli_captains_log)

    cl_read = cl_sub.add_parser("read", help="Show recent Captain's Log entries")
    cl_read.add_argument("--limit", type=int, default=20, help="Maximum entries to display (default: 20)")
    cl_read.set_defaults(func=cli_captains_log)

    cl_clear = cl_sub.add_parser("clear", help="Clear all Captain's Log entries")
    cl_clear.set_defaults(func=cli_captains_log)
    ...
    p_chat.add_argument("--session", type=str, default="cli", help="Session ID for conversation grouping (default: cli)")
    p_chat.add_argument("--rag", action="store_true", help="Enable RAG (retrieval-augmented generation) using a RAG index")
    p_chat.add_argument("--index", type=str, default="docs", help="Name of RAG index to use when --rag is set (default: docs)")
    p_chat.set_defaults(func=cmd_chat, rag=False)
    ...
    p_s_hist.add_argument("--limit", "-n", type=int, default=20, help="Maximum messages to display (default: 20)")
    p_s_hist.set_defaults(func=cli_session_history)
    ...
    p_s_clear.set_defaults(func=cli_session_clear)

    p_s_list = subparsers.add_parser("session-list", help="List known session IDs")
    p_s_list.set_defaults(func=cli_session_list)
