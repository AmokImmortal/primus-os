        ...
        try:
            if hasattr(runtime, "chat_once"):
                reply = runtime.chat_once(
                    ...
                    rag_index=getattr(args, "index", None),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # status
    p_status = add_command("status", help="Show system status")
    p_status.set_defaults(func=cmd_status)

    # self-test
    p_self = add_command("self-test", help="Run self-tests (boot/agents/checks)")
    p_self.set_defaults(func=cmd_self_test)
    p_self_alt = add_command("selftest", help="Run self-tests (boot/agents/checks)")
    p_self_alt.set_defaults(func=cmd_self_test)

    # start/stop
    p_start = add_command("start", help="Start primus runtime")
    p_start.set_defaults(func=cmd_start)
    p_stop = add_command("stop", help="Stop primus runtime")
    p_stop.set_defaults(func=cmd_stop)

    # agent commands
    p_agent = add_command("agent", help="Agent operations")
    ...
    # RAG commands
    p_rag = add_command("rag", help="RAG operations")
    ...
    p_rag_index = add_command("rag-index", help="Index a path into RAG")
    ...
    p_rag_search_direct = add_command("rag-search", help="Search a RAG index")
    ...
    # subchats
    p_subchat = add_command("subchats", help="Subchat operations")
    ...
    # chat
    p_chat = add_command("chat", help="Single-turn chat or interactive REPL if no message is provided")
    ...
    # logs
    p_logs = add_command("logs", help="Show recent CLI logs")
    p_logs.set_defaults(func=cmd_logs)

    # debug
    p_debug = add_command("debug", help="Developer debug info")
