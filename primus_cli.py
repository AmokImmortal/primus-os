        ...
        try:
            ...
            if core and hasattr(core, "chat"):
                reply = core.chat(
                    ...
                    rag_index=args.index if args.rag else None,
                ...
                reply = runtime.chat_once(
                    ...
                    rag_index=getattr(args, "index", None) if getattr(args, "rag", False) else None,
                    max_tokens=getattr(args, "max_tokens", 256),
                )
                ...
                reply = runtime.chat_with_options(
                    user_message=args.message,
                    session_id=getattr(args, "session", None),
                    index_name=getattr(args, "index", None) if getattr(args, "rag", False) else None,
                    use_rag=bool(getattr(args, "rag", False)),
                    max_tokens=getattr(args, "max_tokens", 256),
                )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_chat.add_argument("--session", type=str, default="cli", help="Optional session identifier to reuse across calls")
    p_chat.add_argument("--index", type=str, default="docs", help="RAG index name to use when --rag is enabled")
    p_chat.add_argument("--rag", action="store_true", help="Enable RAG context for chat")
