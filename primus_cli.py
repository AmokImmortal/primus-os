...
def cli_session_list(args: argparse.Namespace) -> None:
    ...
    Uses PrimusRuntime -> PrimusCore -> SessionManager.list_sessions().
    """
    runtime, _ = get_runtime()
    if not runtime:
        print("Runtime not available. Ensure primus runtime exists.")
    ...
    if hasattr(core, "list_sessions"):
        fn = getattr(core, "list_sessions", None)
        if callable(fn):
            try:
                sessions = fn()
    ...
    if sessions is None:
        ...
        if sm is not None:
            list_fn = getattr(sm, "list_sessions", None)
            if callable(list_fn):
                try:
                    sessions = list_fn()
                except Exception as exc:  # noqa: BLE001
...
def cli_captains_log(args: argparse.Namespace) -> None:
    """Captain's Log controls routed through PrimusCore.captains_log."""
    ...
    try:
        ...
        print(f"Captain's Log operation failed ({action!r}): {exc}")
        return
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_cl.add_argument(
        "action",
        choices=["write", "read", "clear", "status"],
    ...
    p_cl.add_argument(
        ...
        nargs="?",
        default="",
        help="Text for 'write' action; ignored otherwise",
    )
